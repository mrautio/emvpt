...
impl EmvConnection {
    ...
    fn process_tlv(&mut self, buf: &[u8], level: u8) {
        ...
        match tlv_data.value() {
            ...
            Value::Primitive(v) => {
                self.tags.insert(tag_name, v.to_vec());
...
fn read_record(connection : &mut EmvConnection, short_file_identifier : u8, record_index : u8) -> Option<Vec<u8>> {
    ...
    let (response_trailer, response_data) = connection.send_apdu(&read_record);
...
fn read_all_records(connection : &mut EmvConnection, short_file_identifier : u8) -> Option<Vec<u8>> {
    let mut records : Vec<u8> = Vec::new();

    for record_index in 0x01..0xFF {
        match read_record(connection, short_file_identifier, record_index) {
            Some(data) => {
                records.extend(&data);
            },
            None => break
...
fn run() -> Result<Option<String>, String> {
   ...
   //return Ok(None);
    ...
    info!("Selecting Payment System Environment (PSE):");
    let contact_pse_name      = "1PAY.SYS.DDF01";

    let pse_name = contact_pse_name;

    let (response_trailer, response_data) = connection.send_apdu_select(&pse_name.as_bytes());
    if !is_success_response(&response_trailer) {
        return Err(format!("Could not select {:?}", pse_name));
    }

    let sfi_data = connection.get_tag_value("88").unwrap();
    assert_eq!(sfi_data.len(), 1);
    let short_file_identifier = sfi_data[0];

    info!("Read available AIDs:");

    let all_records = read_all_records(&mut connection, short_file_identifier).unwrap();
    let tlv_data = parse_tlv(&all_records[..]);

    // TODO: make selecting possible

    let tlv_tag = find_first_tag("61", &tlv_data).unwrap();
    let aid_data = parse_tag_value("4F", &tlv_tag).unwrap();
    let label_data = parse_tag_value("50", &tlv_tag).unwrap();
    //let pdol_data = parse_tag_value("9F38", &tlv_tag).unwrap();
    info!("Selecting application. AID:{:02X?}, label:{:?}", aid_data, str::from_utf8(&label_data).unwrap());
    let (response_trailer, _) = connection.send_apdu_select(&aid_data);
    if !is_success_response(&response_trailer) {
        return Err(format!("Could not select {:?}", pse_name));
    }

    info!("GET PROCESSING OPTIONS:");
    let get_processing_options_command = b"\x80\xA8\x00\x00\x02\x83\x00".to_vec();
    let (response_trailer, response_data) = connection.send_apdu(&get_processing_options_command);
    if !is_success_response(&response_trailer) {
        return Err(format!("Could not get processing options"));
    }

    // TODO convert format 1 to format 2, support format 2
    assert_eq!(response_data[0], 0x80); // currently only format 1 supported
    ...
    // 80 | 0E | 3C 00 | 08 01 01 00, 10 01 03 00, 18 01 02 01

    let tag_82_aip = &response_data[2..4];
    ...
    let tag_94_afl = &response_data[4..];
    ...
    for i in (0..tag_94_afl.len()).step_by(4) {
        ...
        for record_index in record_index_start..record_index_end+1 {
            if let Some(data) = read_record(&mut connection, short_file_identifier, record_index) {
                ...
                if data_authentication_records > 0 {
                    ...
                    let mut data_authentication_data;
    ...
    let tag_92_issuer_pk_remainder = connection.get_tag_value("92").unwrap();
    let tag_9f32_issuer_pk_exponent = connection.get_tag_value("9F32").unwrap();
    let tag_90_issuer_public_key_certificate = connection.get_tag_value("90").unwrap();

    let rid = &aid_data[0..5];
    let tag_8f_ca_pk_index = connection.get_tag_value("8F").unwrap();
    ...
    assert_eq!(issuer_certificate[1], 0x02);
    ...
    let tag_5a_pan = connection.get_tag_value("5A").unwrap();
    ...
    let issuer_pk = RsaPublicKey::new(&issuer_pk_modulus[..], &tag_9f32_issuer_pk_exponent[..]);
    ...
    assert_eq!(icc_certificate[1], 0x04);
    ...
    let tag_9f47_icc_pk_exponent = connection.get_tag_value("9F47").unwrap().clone();
    ...
    let tag_9f4a_static_data_authentication_tag_list = connection.get_tag_value("9F4A").unwrap();
    // TODO: handle list
    ...
    info!("ICC PK modulus ({} bytes):\n{}", icc_pk_modulus.len(), HexViewBuilder::new(&icc_pk_modulus[..]).finish());

    info!("Perform Dynamic Data Authentication (DDA):");

    let tag_9f49_ddol = connection.get_tag_value("9F49").unwrap();
    ...
    let (response_trailer, response_data) = connection.send_apdu(&internal_authenticate_command);
    if !is_success_response(&response_trailer) {
        return Err(format!("Could not process internal authenticate"));
    }

    assert_eq!(response_data[0], 0x80); // TODO: currently only format 1 supported
    let tag_9f4b_signed_data = &response_data[3..];
    info!("9F4B signed data result moduluslength:{}, ({} bytes):\n{}", icc_pk_modulus.len(), tag_9f4b_signed_data.len(), HexViewBuilder::new(&tag_9f4b_signed_data[..]).finish());

    let icc_pk = RsaPublicKey::new(&icc_pk_modulus[..], &tag_9f47_icc_pk_exponent[..]);
    ...
    if ascii_pin.is_some() {
        ...
        let ascii_pin = ascii_pin.unwrap();

        let pin_bcd_nc = ascii_to_bcd_cn(&ascii_pin.as_bytes(), 6).unwrap();
        ...
        let (response_trailer, response_data) = connection.send_apdu(&verify_command);
        if !is_success_response(&response_trailer) {
            //Incorrect PIN = 63, C4
            return Err(format!("Could not process PIN verify"));
        }
    }

    info!("Generate Application Cryptogram (GENERATE AC):");
    ...
    let (response_trailer, response_data) = connection.send_apdu(&generate_ac_command);
    if !is_success_response(&response_trailer) {
        return Err(format!("Could not process generate ac"));
    }

    let tag_9f27_cryptogram_information_data = &response_data[2];
    let tag_9f36_application_transaction_counter = &response_data[3..5];
    let tag_9f26_application_cryptogram = &response_data[5..13];
    let tag_9f10_issuer_application_data = &response_data[13..]; // TODO: this is optional, should check size
